...
class OneByOne(nn.Module):
  def __init__(self):
    ...
    self.op = nn.Conv2d(2048, 1024, padding=0, stride=1, kernel_size=1)
...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    for p in self.RCNN_base_2[0].parameters(): p.requires_grad=False
    for p in self.RCNN_base_2[1].parameters(): p.requires_grad=False

    assert (0 <= cfg.RESNET.FIXED_BLOCKS < 4)
    if cfg.RESNET.FIXED_BLOCKS >= 3:
      for p in self.RCNN_base_1[6].parameters(): p.requires_grad=False
      for p in self.RCNN_base_2[6].parameters(): p.requires_grad=False
    if cfg.RESNET.FIXED_BLOCKS >= 2:
      for p in self.RCNN_base_1[5].parameters(): p.requires_grad=False
      for p in self.RCNN_base_2[5].parameters(): p.requires_grad=False
